...
const TableAddAttribute = ({
	...
	const getListAttributeName = async keyword => {
		...
		let tmpArr1 = attrCurrent.filter(
			item => !listAttribute.find(i => i.type === item.type) && !uniqueAttrNameBefore.find(i => i.type === item.type)
		);
	...
	return (
		<div className={cx("table")}>
			...
			{listAttribute.map((item, idx) => (
				<Grid container className={cx("data")} key={`attr-${idx}`}>
					<Grid item xs={2} className={cx("data-cell")} style={{ flexDirection: "column", alignItems: "flex-start" }}>
						<Dropdown
							...
							<div className={cx("drop-down-label")}>
								<input
									...
									className={cx(
										"input",
										(listAttributeError[idx]?.nameDuplicate || listAttributeError[idx]?.nameNull) && "invalid"
									)}
									onChange={e => {
						...
						{(listAttributeError[idx]?.nameDuplicate && <div style={{ color: "#9e494d" }}>{"Duplicate"}</div>) ||
					...
					<Grid item xs={5} className={cx("data-cell")} style={{ flexDirection: "column", alignItems: "flex-start" }}>
						<div className={cx("content", listAttributeError[idx]?.valueNull && "invalid")}>
							<Dropdown
								...
								disabled={listAttributeError[idx]?.disable}>
								<div className={cx("drop-down-label")}>
									<span>{item?.valueType || "Type"}</span> {!listAttributeError[idx]?.disable && <DownOutlined />}
								</div>
							</Dropdown>
