...
import pendding from "src/assets/icons/pendding1.png";
							...
							<Grid container className={cx("main-row")} style={{ alignItems: isSubmitted ? "flex-start" : "center" }}>
									...
									{listErr[index].tokenIdErr &&
										(() => {
											...
											if (isNaN(item.tokenId) || item.tokenId < 0) {
												return <div style={{ color: "#9e494d" }}>TokenId must be a positive number!</div>;
											}
								...
								<Grid item xs={2}>
									<input
										...
										className={cx(
											"input",
											listErr[index].supplyErr && (!item.supply || isNaN(item.supply)) && "invalid"
										)}
										onChange={e => {
									...
									<Grid container style={{ alignItems: "center" }}>
										...
										<Grid item xs={7}>
											<input
												...
												value={item?.imageUrl}
												className={cx("input", listErr[index].imageUrlErr && !item.imageUrl && "invalid")}
												onChange={e => {
													setImageIdx(index);
			...
			<Modal
				...
				<Modal.Body className={cx("modal-body")}>
					...
					<div className={cx("center-body")}>
						<img src={pendding} alt="pendding" className={cx("icon-list")} />
